...
class AgentService:
    """Manus 智能体服务（接入新 CodingAgent runtime）"""
    ...
    async def _create_task(self, session: Session) -> Task:
        """根据传递的会话创建一个新任务"""
        # 创建 AgentTaskRunner（新 runtime 无需 sandbox/browser）
    ...
    async def _safe_update_unread_count(self, session_id: str) -> None:
        ...
        该方法通过 asyncio.create_task() 调用，运行在一个全新的 asyncio Task 中，
        因此不受 sse_starlette 的 anyio cancel scope 影响，数据库操作可以正常完成。
        使用 uow_factory 创建全新的 UoW 实例，避免与被取消的上下文共享数据库连接。
        """
        try:
    ...
    async def chat(
        ...
        try:
            ...
            if message:
                ...
                # 7. 创建一个人类消息事件（attachments 在新 runtime 中暂不处理）
                if attachments:
                    logger.warning(
                        "File attachments provided but not yet supported in CodingAgent mode — %d attachment(s) ignored",
                        len(attachments)
                    )
                ...
                # 8. 将事件添加到任务的输入流中，好让 Agent 获取到数据
            ...
            while task and not task.done:
                ...
                # 13. 使用 Pydantic 提供的类型适配器将 event_str 转换为指定类实例
            ...
            # 16. 循环外面表示这次任务 AI 端已结束
            logger.info(f"会话[{session_id}]本轮运行结束")
        except Exception as e:
            # 17. 记录日志并返回错误事件
            logger.error(f"任务会话[{session_id}]对话出错: {e}")
            ...
            # 18. 将未读消息数重置（在独立 Task 中执行，避免 anyio cancel scope 干扰）
            try:
